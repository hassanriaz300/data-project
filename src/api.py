...
# ─────────────────────────────────────────────────────────────────────────────
# 1) Compute path to React’s production build
...
model_bundle: ModelBundle  # will be set in lifespan
...
async def prepare_endpoint(file: UploadFile = File(...)):
    ...
    # Determine target filename, avoid overwriting existing
...
# ─────────────────────────────────────────────────────────────────────────────
# 2) Mount the React build at “/” so that:
#    • GET /               → serves build/index.html
#    • GET /static/...     → serves JS/CSS/images
#    • Any other path     → falls through to your API routes
