# src/services/visualizationservice.py
...
def _map_value(val):
    """
    Map a single accusation string to its unified category label.
    """
...
def _normalize_list(acc_list):
    """
    Map each accusation in a list to its unified category label.
    """
...
def plot_top10_accusation_heatmap(input_path: str) -> dict:
    """
    Loads Excel with accusation tiers, normalizes similar accusations,
    creates a heatmap of top 10 accusations by frequency in each group,
    and saves a PNG plot.
    Returns metadata about the result.
    """
    df = pd.read_excel(input_path)

    # 1. Parse and normalize list-like columns
...
def plot_top10_by_rank_heatmap(input_path: str) -> dict:
    ...
    # Melt Top1–Top5 into long format
    records = []
...
def plot_semantic_topic_heatmap(input_path: str) -> dict:
    ...
    # Melt SemTop1–SemTop5 into long format
    records = []
    for pos in range(1, 6):
        ...
        if col in df.columns:
            ...
            temp["position"] = pos
            # map each topic string into your category map if it matches exactly
...
def plot_top5_accusations(input_path: str) -> dict:
    ...
    # 2. Gather all (accusation, score) pairs, mapping each to its category
...
def get_top1_category_trends(input_path: str) -> dict:
    ...
    # figure out each review's absolute top1 label + map to category
    out = []
    for _, row in df.iterrows():
        pairs = gather_pairs(row)
        if not pairs:
    ...
    # pivot into chart-friendly form
...
def get_store_benchmarks(input_path: str) -> dict:
    ...
    Returns for each city:
      - city
      - avg_rating
      - complaint_diversity (# distinct Top-1 categories)
      - top1_category (most common Top-1)
    """
    df = pd.read_excel(input_path)
    df["date"] = pd.to_datetime(df["date"])
...
def generate_wordclouds(input_path: str):
    """
    Returns: dict[category] = path_to_wordcloud.png
    """
...
def drilldown_analysis(input_path: str, group_by: str, group_value: str = None):
    """
    Returns drilldown stats for city/store/address. If group_value is None, returns all groups.
    """
    df = pd.read_excel(input_path)
    if group_by not in df.columns:
        raise ValueError(f"Grouping column '{group_by}' not found in uploaded file")
    ...
    # Top accusation by mapped category (using evidence if available, else fall back to Top1)
    ...
    for _, row in df.iterrows():
        ...
        if isinstance(raw, str):
            ...
            # Fallback: use Top1 column (if evidence missing)
            top1 = row.get("Top1")
            if pd.notna(top1):
                cat = _map_value(top1)
...
def deep_analyze_service(
    ...
    # Try evidence-based wordclouds first
    ...
    # Fallback: If no evidence-based wordclouds, use review-text wordclouds
    if not wordclouds:
...
def generate_wordclouds_from_top_accusation(
    ...
    """
    Generate wordclouds from review texts, grouped by each review's top accusation (using gather_pairs).
    """
    df = pd.read_excel(input_path)
    ...
    # Group texts by their Top1 category
    cat_to_texts = defaultdict(list)
    for _, row in df.iterrows():
        pairs = gather_pairs(row)
        if not pairs:
            continue
        top1 = sorted(pairs, key=lambda x: x[1], reverse=True)[0][
            0
