# src/services/semanticservice.py
# ──────────────────────────────────────────────────────────────────────────
"""
Semantic accusation mapper

Replicates the exact logic of `semantic_accusation_mapper_v2.py`, but wrapped
in a callable `map_accusations()` function and a CLI – matching the style of
src/services/cleaning.py.
...
Example (Python):
    ...
    map_accusations(
        ...
        output_path="data/interim/top5_semantic_mapped.xlsx"
    )

Example (CLI):

    python -m src.services.semanticservice \
           --input  data/interim/with_top5_accusations.xlsx  \
           --output data/interim/top5_semantic_mapped.xlsx
"""
...
# -------------------------------------------------------------------------
# 1) Defaults & hyper-parameters  (edit as you wish)
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# 2) Static tables  (verbatim from the original notebook)
# -------------------------------------------------------------------------
ALL_ACCS: List[str] = [
    # … full list kept identical …
...
# -------------------------------------------------------------------------
# 3) Helper functions  (same behaviour, just pulled into callables)
# -------------------------------------------------------------------------
def _as_list(x):
    if x is None:
        return []
...
def _get_candidate_accs(theme, catex, cat) -> List[str]:
    """Heuristic narrowing of label universe – identical to notebook."""
    cands: List[str] = []
...
# -------------------------------------------------------------------------
# 4) Public API – identical results, just a function call
# -------------------------------------------------------------------------
def map_accusations(
    ...
    """
    Attach semantic Top-N accusation columns to *input_path* Excel
    and write the enriched DataFrame to *output_path*.

    The algorithm is exactly the same as in semantic_accusation_mapper_v2.py.
    """
...
# -------------------------------------------------------------------------
# 5) CLI  (python -m src.services.semanticservice ...)
# -------------------------------------------------------------------------
